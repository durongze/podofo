...
{
    PdfString sJap(reinterpret_cast<const pdf_utf8*>("「PoDoFo」Japanese"));
    PdfAnnotation* pAnnotation =
        pPage->CreateAnnotation( ePdfAnnotation_Text, PdfRect( 400.0, 200.0, 20.0, 20.0 ) );

    PdfString sGerman(reinterpret_cast<const pdf_utf8*>("Unicode Umlauts: German"));
...
int main( int argc, char* argv[] )
{
    try {
        ...
        TEST_SAFE_OP( writer.GetInfo()->SetSubject (
                          PdfString(reinterpret_cast<const pdf_utf8*>("「PoDoFo」Japanese") ) ) );
